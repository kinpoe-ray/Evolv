...
const AlumniHub: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">连接校友网络，分享职场经验，寻找导师指导，获取求职资源</p>
          </div>
          {activeTab === 'posts' && (
            <button
      ...
      {activeTab === 'mentors' && (
        <div className="space-y-6">
          ...
          {filteredMentors.map((mentor) => {
            ...
            const myRequest = myMentorRequests.find(r => r.mentor_id === mentor.id)
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center mb-3">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900">{mentor.mentor_name}</h3>
                  ...
                  <div className="ml-6">
                    {myRequest ? (
                      <div className={`px-4 py-2 rounded-lg text-sm font-medium ${
                        ...
                        myRequest.status === 'accepted' ? 'bg-green-100 text-green-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {myRequest.status === 'pending' ? '申请中' :
                         myRequest.status === 'accepted' ? '已接受' : '已拒绝'}
                      </div>
                    ) : (
                      <button
                        onClick={() => {
            ...
            <div key={resource.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="flex space-x-2">
                {resource.file_url ? (
                  ...
                  <button className="flex-1 flex items-center justify-center px-4 py-2 bg-gray-300 text-gray-500 rounded-lg cursor-not-allowed">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-900">分享职场经验</h3>
              <button
      ...
      {/* 申请导师弹窗 */}
      {showApplyMentor && selectedMentor && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-900">申请导师指导</h3>
              <button
