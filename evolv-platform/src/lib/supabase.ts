...
// 使用环境变量，fallback到默认值（仅用于开发）
const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || "https://qpgefcjcuhcqojiawpit.supabase.co";
const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFwZ2VmY2pjdWhjcW9qaWF3cGl0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjE5OTk5MjgsImV4cCI6MjA3NzU3NTkyOH0.CJJCNIYH2FjA83lJ1UhJbTZeDD41_nvEEq2gsz9sqLg";

// 验证环境变量
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error('缺少必要的Supabase环境变量。请检查.env文件中的VITE_SUPABASE_URL和VITE_SUPABASE_ANON_KEY');
}
...
export type Profile = {
  id: string;
  user_type: 'student' | 'teacher' | 'alumni';
  full_name: string;
  avatar_url?: string;
  school?: string;
  major?: string;
